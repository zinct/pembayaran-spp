...
@section('content')
  <div class="row">
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          <form action="{{ route('admin.data.spp.update', $spp->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <div class="form-group row mb-4">
              <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Nama Pembayaran</label>
              <div class="col-sm-12 col-md-7">
                <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" value="{{ old('nama', $spp->nama) }}" placeholder="ketik nama spp">
                @error('nama')
            ...
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-12 col-md-7">
                <select type="text" name="tahun_id" id="tahun_id" class="form-control @error('tahun_id') is-invalid @enderror">
                  <option value="">Pilih Tahun Ajaran</option>
                  @foreach ($tahun as $row)
                    @if ($row->id == old('tahun_id', $spp->tahun_id))
            ...
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-12 col-md-7">
                <input type="text" class="form-control numeric @error('nominal') is-invalid @enderror" name="nominal" value="{{ old('nominal', $spp->nominal) }}" placeholder="ex. 5.000.000">
                @error('nominal')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
            ...
            <div class="form-group row mb-4">
              ...
              <div class="col-sm-12 col-md-7">
                ...
                @foreach ($kelas as $row)
                  @if ($spp->kelas->contains($row->id))
                    <div class="custom-control custom-checkbox">
                      <input type="checkbox" name="kelas_id[]" value="{{ $row->id }}" id="kelas-{{ $row->id }}" class="custom-control-input" checked>
